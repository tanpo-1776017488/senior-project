...
class mtcnn_custom():
    def __init__(self,device,p_model_path,r_model_path,o_model_path):
        self.device=device
        self.p_net=PNet().to(self.device)
        self.r_net=RNet().to(self.device)
        self.o_net=ONet().to(self.device)

        self.p_net.load_state_dict(torch.load(p_model_path, map_location=lambda storage, loc: storage))
        self.r_net.load_state_dict(torch.load(r_model_path, map_location=lambda storage, loc: storage))
        self.o_net.load_state_dict(torch.load(o_model_path, map_location=lambda storage, loc: storage))

        self.p_net.eval()
        self.r_net.eval()
        self.o_net.eval()

    def detect_all_net(self,image,mini_face):
        boxes = np.array([])
        landmarks = np.array([])
        bboxes=detect_pnet(self.p_net,image,mini_face,self.device)
        bboxes = detect_rnet(self.r_net, image, bboxes, self.device)
        bboxes, landmarks = detect_onet(self.o_net, image, bboxes, self.device)
