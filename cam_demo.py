...
if __name__ == '__main__':
    ...
    parser.add_argument('-e','--embedding',type=int,default=512,help='embedding size')
    args = parser.parse_args()
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    #prepare pretrained model
    detect_model = MobileFaceNet(args.embedding).to(device)  # embeding size is 512 (feature vector)
    check_point=torch.load('Weights/model_final.pth',map_location=lambda storage, loc: storage)
        ...
        if isSuccess:
            try:
                ...
                for i, b in enumerate(bboxes):
                    ...
                    print("x1 : {},y1 : {}, x2 : {}, y2: {}".format(b[0],b[1],b[2],b[3]))
                    if args.score:
